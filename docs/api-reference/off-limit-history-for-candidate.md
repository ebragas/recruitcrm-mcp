<!-- source: https://docs.recruitcrm.io/docs/rcrm-api-reference/7a2de851b6e2f-off-limit-history-for-candidate -->
<!-- title: Off Limit History for Candidate | API Endpoints -->

# Off Limit History for Candidate

**GET** `/v1/candidates/{candidate}/off-limit-history`

Returns the off-limit history for the requested candidate.

**Note:** If the candidate was marked as off-limit in the past but is currently not off-limit, the latest status will be available along with the corresponding 'updated\_on' timestamp.
...
```
curl --request GET \
  --url https://api.recruitcrm.io/v1/candidates/{candidate}/off-limit-history \
