<!-- source: https://docs.recruitcrm.io/docs/rcrm-api-reference/0ed92e11f32ef-opted-out-entities -->
<!-- title: Opted out entities | API Endpoints -->

# Opted out entities

**GET** `/v1/email/opted-out`

Get list of opted out entities.
...
- `related_to_type` (string, **required**) — Entity Type i.e. candidate/ contact Examples:
- `candidate` (optional)
...
- `email_opt_out_source` (string, optional) — Opted out source. Manually opted out or Unsubscribed Not Setcreatedonupdatedon select an option (defaults to: updatedon) sort\_order: Not Setascdesc select an option (defaults to: desc) (example: `Manually opted out`)
...
```
curl --request GET \
  --url 'https://api.recruitcrm.io/v1/email/opted-out?related_to_type=candidate' \
