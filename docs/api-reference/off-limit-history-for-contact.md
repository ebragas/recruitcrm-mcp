<!-- source: https://docs.recruitcrm.io/docs/rcrm-api-reference/4af4eabecdfa3-off-limit-history-for-contact -->
<!-- title: Off Limit History for Contact | API Endpoints -->

# Off Limit History for Contact

**GET** `/v1/contacts/{contact}/off-limit-history`

Returns the off-limit history for the requested contact.

**Note:** If the contact was marked as off-limit in the past but is currently not off-limit, the latest status will be available along with the corresponding 'updated\_on' timestamp.
...
```
curl --request GET \
  --url https://api.recruitcrm.io/v1/contacts/{contact}/off-limit-history \
